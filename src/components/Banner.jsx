...
const Banner = () => {
  return (
    <main className="h-[110vh] w-full bg-[url('/images/programming-bg.jpg')] bg-cover bg-fixed clip-path flex flex-col justify-center md:flex-row md:items-center md:justify-normal px-5 sm:px-8 md:px-12 lg:px-32 xl:px-40" id="navbar">
        ...
        <div>
          {/* Code for tailwind gradient text */}
            {/* bg-gradient-to-br from-THEME_PINK to-THEME_BLUE inline-block bg-clip-text text-transparent */}
            <h4 className="text-white text-lg md:text-xl xl:text-2xl font-semibold mb-2">Hi, My name is</h4>
            <h1 className="text-3xl md:text-4xl xl:text-5xl font-bold mb-2 xl:mb-3">Ahmed Farooq</h1>
            <h2 className={`${play.className} text-2xl xl:text-3xl font-semibold mb-2 xl:mb-3 inline-block`}>I&apos;m a <Typewriter words={['Software Developer', 'Frontend Developer', 'MERN Stack Developer']} loop={0} /></h2>
            ...
            <div className="flex gap-2 pt-4">
              ...
              <Link href="/contact" className="bg-transparent text-black outline-[#100028] outline outline-2 font-bold rounded flex justify-center items-center w-36 py-3">Contact Me</Link>
            </div>
          </div>
        </div>
